...
function uc_fopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	...
	!isset($matches['scheme']) && $matches['scheme'] = 'http';
	!isset($matches['host']) && $matches['host'] = '';
	!isset($matches['path']) && $matches['path'] = '';
	!isset($matches['query']) && $matches['query'] = '';
	!isset($matches['port']) && $matches['port'] = '';
    $host = $matches['host'];
    $_host = empty($host) ? $host : ($matches['scheme'] == 'https' ? "ssl://{$host}" : $host);

    /*error_log("host:{$host} [" . date('Y-m-d H:i:s') . "]\n", 3, "./client.log");
    error_log("_host:{$_host} [" . date('Y-m-d H:i:s') . "]\n", 3, "./client.log");*/
	...
	if(function_exists('fsockopen')) {
        /*$fp = @fsockopen(($ip ? $ip : "ssl://".$host), $port, $errno, $errstr, $timeout);*/
        $fp = @fsockopen(($ip ? $ip : $_host), $port, $errno, $errstr, $timeout);
	} elseif (function_exists('pfsockopen')) {
        /*$fp = @pfsockopen(($ip ? $ip : "ssl://".$host), $port, $errno, $errstr, $timeout);*/
        $fp = @pfsockopen(($ip ? $ip : $_host), $port, $errno, $errstr, $timeout);
	} else {
		$fp = false;
	}
	if(!$fp) {
